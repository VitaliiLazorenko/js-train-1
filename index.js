...
function getNumberFromString(s) {
  let number = parseFloat(s); // Використовуємо функцію parseFloat для того, щоб спробувати отримати число з рядка
  if (isNaN(number)) {
...
function getIntegerFromString(s) {
  let number = parseInt(s); // Використовуємо функцію parseInt для того, щоб спробувати отримати ціле число з рядка
  if (isNaN(number)) {
    // Перевіряємо, чи є отримане значення числом за допомогою isNaN
...
function createURLObject(urlStr) {
  ...
  return new URL(urlStr); // Повертаємо створений об'єкт URL.
}
